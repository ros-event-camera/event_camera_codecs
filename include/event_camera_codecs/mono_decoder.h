...
#ifndef EVENT_CAMERA_CODECS__MONO_DECODER_H_
#define EVENT_CAMERA_CODECS__MONO_DECODER_H_
...
public:
    ...
    uint64_t * nextTime) override
  {
    for (const uint8_t * p_u8 = buf; p_u8 < buf + bufSize; p_u8 += 8) {
      ...
      processor->eventCD(
        t, static_cast<uint16_t>((p >> 32) & 0xFFFFULL),
        static_cast<uint16_t>((p >> 48) & 0x7FFFULL),
        static_cast<bool>(p & ~0x7FFFFFFFFFFFFFFFULL));
    }
    processor->finished();
    return (bufSize);
...
#endif  // EVENT_CAMERA_CODECS__MONO_DECODER_H_
